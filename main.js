...
import { getStoredApiKey, setStoredApiKey } from './openai.js';
import { extractTextFromResumeFile, parseExperiencesWithOpenAI } from './resumeParser.js';
    ...
    <header class="topnav no-print">
      <div class="topnav__inner">
        ...
        <div class="topnav__actions">
          <button class="icon-btn" type="button" aria-label="用户">${icon('user')}</button>
...
function renderHomePage() {
  ...
  const statCards = [
    { icon: '✅', color: 'green', label: '已完成经历分析数', value: entries.length, hint: '建议至少 3 段，可提升分析稳定性。' },
...
async function askMentorAgent(userText, mentor) {
  ...
  if (!res.ok) throw new Error('mentor chat api failed');
  const data = await res.json();
  return data?.choices?.[0]?.message?.content?.trim() || '我再追问你一个细节：这件事里你最在意的是成长速度、稳定，还是收入？';
}
...
async function parseResumeAndFill() {
  ...
  try {
    ...
    const parsed = await parseExperiencesWithOpenAI(text, apiKey);
...
function validateReflection() {
  if (!coachMessages.some((m) => m.role === 'user')) {
...
async function askCoachAI(nextUserText) {
  ...
  if (!apiKey) {
    ...
    const asked = coachMessages.filter((m) => m.role === 'assistant').length - 1;
  ...
  return text || '你可以继续补充一个成就事件，我会据此继续追问。';
}
...
async function summarizeReflectionFromChat() {
  ...
  if (!apiKey) {
    ...
    reflectionForm = {
      ...
      testReference: reflectionForm.testReference || '本次使用对话提炼（未调用测评）',
    };
    return;
  }
    ...
    messages: [
      {
        role: 'system',
        content:
          '你是职业偏好总结器。根据用户对话输出 JSON：{"achievementRecall":"","values":[],"personality":{"introExtro":"","workStyle":""},"testReference":""}。values 仅可从[成就感,稳定性,创新探索,平衡生活,高收入]中选择。',
      },
      {
        role: 'user',
        content: coachMessages.map((m) => `${m.role === 'assistant' ? '教练' : '用户'}：${m.content}`).join('\n'),
      },
    ],
  };
  ...
  reflectionForm = {
    achievementRecall: String(parsed.achievementRecall || '').trim() || userText.slice(0, 260),
    values: Array.isArray(parsed.values) ? parsed.values.slice(0, 5) : ['成就感'],
    personality: {
      introExtro: parsed.personality?.introExtro || '均衡',
      workStyle: parsed.personality?.workStyle || '均衡',
    },
    testReference: parsed.testReference || 'AI 对话提炼（MBTI/霍兰德可作参考）',
  };
}
...
function renderReflectionStep() {
  ensureCoachSession();
  const keyExists = !!getStoredApiKey();
  return `
    <section class="panel">
      <h2>步骤 2 / 6 · 职业偏好校准</h2>
      ...
      <div class="chat-wrap">
        ...
        <div class="toolbar">
          ...
          <button type="button" class="btn btn--ghost btn--sm" id="btn-save-coach-key">保存 API Key</button>
          <button type="button" class="btn btn--ghost btn--sm" id="btn-clear-coach-key">清除 Key</button>
          <button type="button" class="btn btn--primary" id="btn-send-coach" ${coachLoading ? 'disabled' : ''}>发送</button>
        </div>
        <p class="small muted">${keyExists ? '已检测到本地 API Key' : '未设置 API Key：将使用本地追问模板'}</p>
      </div>
      <div class="toolbar">
...
async function fetchAgentAdvantageMap(jobA, jobB, factors) {
  ...
  const factorSchema = factors.map((f) => `${f.key}:${f.title}`).join(', ');
  const prompt = `你是联网职业分析agent。请基于公开求职信息与行业认知，对比两个岗位并返回JSON。A=${jobA.name}, B=${jobB.name}。只输出JSON对象，键必须来自[${FACTOR_KEYS.join(
    ','
  )}]。每个键结构：{"lean":"a|b|tie","reason":"不超过40字理由"}。至少覆盖12个键。因素定义：${factorSchema}`;
  const res = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    ...
    body: JSON.stringify({
      ...
      temperature: 0.3,
      messages: [
        { role: 'system', content: '你是会联网检索公开信息的职业对比分析agent。严格返回JSON，不要markdown。' },
  ...
  if (start < 0 || end <= start) throw new Error('bad_agent_json');
  const parsed = JSON.parse(text.slice(start, end + 1));
  const out = {};
  for (const key of FACTOR_KEYS) {
    const item = parsed[key];
    if (!item) continue;
    if (!['a', 'b', 'tie'].includes(item.lean)) continue;
    out[key] = { lean: item.lean, reason: String(item.reason || '').slice(0, 80) };
  }
  if (Object.keys(out).length < 8) throw new Error('agent_result_too_small');
  return out;
}
...
function updateCompareGraphFocus(nodeEl) {
  if (!nodeEl) return;
  ...
  const lean = nodeEl.getAttribute('data-lean') || 'tie';
  ...
  if (icon) icon.textContent = nodeEl.getAttribute('data-icon') || '';
  if (title) title.textContent = nodeEl.getAttribute('data-title') || '';
  if (badge) badge.textContent = nodeEl.getAttribute('data-badge') || '';
  if (aText) aText.textContent = nodeEl.getAttribute('data-a-text') || '';
  if (bText) bText.textContent = nodeEl.getAttribute('data-b-text') || '';
  if (insight) insight.textContent = nodeEl.getAttribute('data-insight') || '';
}
...
function prepareSimulatorForRole(name) {
  ...
  if (sliceCfg) {
    ...
    sliceSimState = null;
    simulator = generateCareerSimulator(job, report.profile);
  }
  simAnswers = {};
  simResult = null;
  return true;
...
function exportWord() {
  ...
  const lines = [name, contact, `求职意向：${targetRole}`, '', skills ? `核心技能：${skills}` : '', summary ? `个人总结：${summary}` : '', expItems].filter(Boolean).join('\n');
    ...
    <section class="panel action-section">
      ...
      <div class="step5-layout">
        <div class="step5-left">
          ...
          <button type="button" class="btn btn--ghost btn--sm" id="btn-export-plan" style="margin-top:0.5rem">
            导出行动计划
          </button>
        </div>
        ...
        ${
          ...
          <div class="resume-parse-box">
            ...
            <div class="field">
              ...
              <input id="resume-api-key" type="password" placeholder="${keyExists ? '已保存，留空不修改' : '请输入 sk-...'}" />
            </div>
            <div class="toolbar">
              <button type="button" class="btn btn--ghost btn--sm" id="btn-save-resume-key">保存 Key</button>
              <button type="button" class="btn btn--ghost btn--sm" id="btn-clear-resume-key">清除 Key</button>
...
function bind() {
  ...
  const saveCoachKeyBtn = document.getElementById('btn-save-coach-key');
  if (saveCoachKeyBtn) {
    saveCoachKeyBtn.addEventListener('click', () => {
      const key = window.prompt('请输入 OpenAI API Key（sk-...）');
      if (!key) return;
  ...
  const clearCoachKeyBtn = document.getElementById('btn-clear-coach-key');
  if (clearCoachKeyBtn) {
    clearCoachKeyBtn.addEventListener('click', () => {
      setStoredApiKey('');
      toast('API Key 已清除');
      render();
    });
  ...
  const saveResumeKey = document.getElementById('btn-save-resume-key');
  if (saveResumeKey) {
    saveResumeKey.addEventListener('click', () => {
      const val = document.getElementById('resume-api-key')?.value?.trim();
      if (!val) {
        toast('请输入 Key');
        return;
      }
  ...
  const clearResumeKey = document.getElementById('btn-clear-resume-key');
  if (clearResumeKey) {
    clearResumeKey.addEventListener('click', () => {
      setStoredApiKey('');
      const input = document.getElementById('resume-api-key');
